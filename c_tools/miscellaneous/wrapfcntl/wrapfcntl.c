...
{
	return lock_reg_special(fd, F_SETLK, F_RDLCK, offset, whence, len);
}

int readw_lock(int fd, off_t offset, int whence, off_t len)	
{
	return lock_reg_special(fd, F_SETLKW, F_RDLCK, offset, whence, len);
}

int write_lock(int fd, off_t offset, int whence, off_t len)
{
	return lock_reg_special(fd, F_SETLK, F_WRLCK, offset, whence, len);
}

int writew_lock(int fd, off_t offset, int whence, off_t len)	
{
	return lock_reg_special(fd, F_SETLKW, F_WRLCK, offset, whence, len);
}

int un_lock(int fd, off_t offset, int whence, off_t len)
{
	return lock_reg_special(fd, F_SETLK, F_UNLCK, offset, whence, len);
}
